...
(() => {
  ...
  function shell() {
    ...
    const title = { home: 'Color Studio', shop: t('Koleksi warna', 'Shop colors'), product: t('Detail shade', 'Shade detail'), cart: t('Keranjang', 'Your bag'), checkout: t('Checkout simulasi', 'Demo checkout'), about: t('Tentang', 'About'), faq: 'FAQ', custom: t('Pesanan khusus', 'Custom orders') };
    document.title = `${title[pageName]} — Solpaċi / B`;
    const nav = [['index', t('Beranda', 'Home')], ['shop', t('Koleksi warna', 'Shop colors')], ['about', t('Tentang', 'Our story')], ['faq', 'FAQ'], ['custom', t('Pesanan khusus', 'Custom orders')]].map(([file, text]) => `<a href="${url(file)}"${pageName === file ? ' aria-current="page"' : ''}>${text}</a>`).join('');
    document.querySelector('#site-top').innerHTML = `
      ...
      <a class="wa-fab" href="https://wa.me/6288973105489?text=${encodeURIComponent(t('Halo Solpaċi, saya tertarik dengan …', 'Hello Solpaċi, I am interested in …'))}" target="_blank" rel="noopener noreferrer" aria-label="${t('Chat WhatsApp Solpaċi', 'Chat Solpaċi on WhatsApp')}"><svg viewBox="0 0 32 32" width="26" height="26" aria-hidden="true" focusable="false"><path fill="currentColor" d="M16 .4C7.4.4.4 7.4.4 16c0 2.8.7 5.5 2.1 7.9L.2 31.6l7.9-2.1c2.3 1.3 4.9 2 7.6 2 8.6 0 15.6-7 15.6-15.6S24.6.4 16 .4zm0 28.5c-2.4 0-4.7-.6-6.7-1.8l-.5-.3-5 1.3 1.3-4.8-.3-.5C3.6 21 2.8 18.5 2.8 16 2.8 8.7 8.7 2.8 16 2.8c3.4 0 6.6 1.3 9 3.7s3.7 5.6 3.7 9c0 7.3-5.9 13.2-13.2 13.2zm7.2-9.9c-.4-.2-2.3-1.1-2.6-1.2-.4-.1-.6-.2-.9.2-.3.4-1 1.2-1.2 1.5-.2.2-.4.2-.8.1-.4-.2-1.6-.6-3-1.9-1.1-1-1.9-2.2-2.1-2.6-.2-.4 0-.6.2-.8.2-.2.4-.4.6-.6.2-.2.3-.4.4-.6.1-.2 0-.5 0-.7s-.9-2.1-1.2-2.9c-.3-.7-.6-.6-.9-.6h-.7c-.2 0-.6.1-1 .5s-1.4 1.3-1.4 3.2c0 1.9 1.4 3.7 1.6 4 .2.2 2.7 4.2 6.6 5.8.9.4 1.6.6 2.2.8.9.3 1.7.2 2.4.1.7-.1 2.3-.9 2.6-1.8.3-.9.3-1.7.2-1.9-.1-.2-.4-.3-.8-.5z"/></svg></a>`;
    document.querySelector('#site-bottom').innerHTML = `
      ...
      <div><p class="eyebrow">${t('KENALI LEBIH DEKAT', 'A LITTLE CLOSER')}</p><a href="${url('about')}">${t('Tentang Solpaċi', 'About Solpaċi')}</a><a href="${url('faq')}">${t('Bantuan & FAQ', 'Help & FAQ')}</a><a href="${url('custom')}">${t('Pesanan khusus', 'Custom orders')}</a><a href="${url('cart')}">${t('Keranjang demo', 'Demo bag')}</a></div>
  ...
  function custom() {
    const shadeOptions = shades.map(s => `<label class="shade-chip"><input type="checkbox" name="shade" value="${s.id}"><span style="--shade:${s.color}" aria-hidden="true"></span><span>${s.name}</span></label>`).join('');
    main.innerHTML = `${breadcrumb(t('Pesanan khusus', 'Custom orders'))}${heading('CUSTOM / A SOLPAĊI STUDY', t('MAU<br><span>PESANAN</span><br>KHUSUS?', 'A LITTLE<br><span>CUSTOM</span><br>PROJECT?'), t('Untuk gift set, bridesmaid, korporat, atau pesanan dalam jumlah lain. Ceritakan acaranya — kami bantu rekomendasikan shade dan kemasan.', 'Gift sets, bridesmaid favours, corporate packs or other bulk orders. Tell us the occasion and we will help recommend shades and packaging.'))}
      <section class="custom-steps section" aria-label="${t('Langkah pemesanan khusus', 'Custom order steps')}"><article><span class="step-num">01</span><h2>${t('CERITA<br>KEBUTUHAN.', 'TELL US<br>WHAT YOU NEED.')}</h2><p>${t('Isi formulir di bawah: nama, acara, jumlah, dan shade yang dipertimbangkan. Tidak perlu detail lengkap di awal.', 'Fill the form below: name, occasion, quantity and shades in mind. Full detail can come later.')}</p></article><article><span class="step-num">02</span><h2>${t('KAMI<br>KONFIRMASI.', 'WE<br>CONFIRM.')}</h2><p>${t('Tim menghubungi lewat WhatsApp untuk diskusi shade, kemasan, harga dan timeline. Tidak ada komitmen sebelum ACC akhir.', 'Our team reaches out on WhatsApp to discuss shades, packaging, price and timeline. No commitment before final approval.')}</p></article><article><span class="step-num">03</span><h2>${t('PRODUKSI<br>& KIRIM.', 'PRODUCTION<br>& DELIVERY.')}</h2><p>${t('Setelah ACC, produksi dimulai. Estimasi waktu dan ongkir diinformasikan sebelum pembayaran.', 'After approval, production starts. Lead time and shipping cost are shared before payment.')}</p></article></section>
      <section class="section custom-form-section" aria-labelledby="custom-form-title"><div><p class="eyebrow">04 / SEND YOUR REQUEST</p><h2 id="custom-form-title">${t('CERITAKAN<br><span>ACARAMU.</span>', 'SHARE<br><span>YOUR OCCASION.</span>')}</h2><p>${t('Isi beberapa kolom di bawah. Detail lebih lengkap menyusul via WhatsApp — tidak ada data yang dikirim ke server pada preview ini.', 'Fill the fields below. More details follow on WhatsApp — no data is sent to a server in this preview.')}</p><ul class="custom-notes"><li>${t('Estimasi harga diberikan setelah kami tahu jumlah dan shade.', 'Price estimates come after we know quantity and shades.')}</li><li>${t('Minimum order dan lead time dikonfirmasi via WhatsApp.', 'Minimum order and lead time are confirmed on WhatsApp.')}</li><li>${t('Pada preview ini formulir tidak mengirim data — gunakan tombol WhatsApp di kanan bawah.', 'This preview form does not submit data — use the WhatsApp button at the bottom right.')}</li></ul></div><form class="custom-form" onsubmit="event.preventDefault(); document.querySelector('.wa-fab').click();" novalidate><div class="field-row"><label>${t('Nama', 'Name')}<input type="text" name="name" autocomplete="name" required></label><label>${t('Email', 'Email')}<input type="email" name="email" autocomplete="email" required></label></div><div class="field-row"><label>${t('Telepon / WhatsApp', 'Phone / WhatsApp')}<input type="tel" name="phone" autocomplete="tel" required></label><label>${t('Tanggal acara', 'Event date')}<input type="date" name="event-date"></label></div><label class="field-block">${t('Acara & cerita singkat', 'Occasion & a short story')}<textarea name="story" rows="3" placeholder="${t('Contoh: bridesmaid 6 orang, tone pink nude, dikemas pouch, tanggal 12 Nov.', 'Example: 6 bridesmaids, pink-nude tones, pouch packaging, 12 Nov.')}"></textarea></label><label class="field-block">${t('Jumlah', 'Quantity')}<input type="number" name="qty" min="1" max="500" placeholder="${t('Contoh: 12', 'Example: 12')}"></label><fieldset class="field-block"><legend>${t('Shade yang dipertimbangkan', 'Shades in mind')}</legend><div class="shade-chips">${shadeOptions}</div></fieldset><label class="field-block">${t('Catatan tambahan', 'Additional notes')}<textarea name="notes" rows="2" placeholder="${t('Packaging, referensi, budget, dll.', 'Packaging, references, budget, etc.')}"></textarea></label><button class="button" type="submit">${t('Kirim lewat WhatsApp', 'Send via WhatsApp')} ${arrow}</button><p class="small-note">${t('Demo: formulir tidak menyimpan data.', 'Demo: the form does not store data.')}</p></form></section>
      <aside class="section editorial-note"><span class="eyebrow">FOR THE DESIGN REVIEW</span><h2>${t('KONFIRMASI VIA WHATSAPP<br><span>BELUM DIIZINKAN.</span>', 'WHATSAPP HANDOFF<br><span>NOT APPROVED YET.</span>')}</h2><p>${t('Halaman ini menunjukkan struktur halaman pesanan khusus. Konten, shade options, dan tombol WhatsApp menunggu ACC klien. Nomor WhatsApp yang tertera adalah placeholder; ganti sebelum live.', 'This page demonstrates the custom-order structure. Copy, shade options and WhatsApp button are pending client approval. The WhatsApp number is a placeholder; replace before going live.')}</p></aside>`;
  }
  shell();
  ...
  ({ home, shop, product, cart: cartPage, checkout, about, faq, custom }[pageName] || home)();
